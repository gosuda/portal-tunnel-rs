...
- **Phase 4 — partial.** Per [`docs/plans/2026-05-04-004-feat-portal-acme-plan.md`](docs/plans/2026-05-04-004-feat-portal-acme-plan.md). Landed: B1 `DnsProvider` trait + `SecretBox` cred newtypes; B2 local self-signed provider; B3 Cloudflare DNS-01 provider + wiremock test; B4 Route53 DNS-01 provider + wiremock test; B5 Google Cloud DNS provider + wiremock test; B6 `Manager` lifecycle façade with the local-self-signed path fully wired. Pending: the `instant-acme` client wrapper that integrates the four DNS providers under `Manager::ensure_certificate` for the three ACME modes — currently `AcmeCloudflare`/`AcmeRoute53`/`AcmeGcloud` all return `AcmeError::Config "not implemented in Phase 4 B6 (waits on B3-B5 + ACME client)"` (`crates/portal-acme/src/manager.rs:154`). Removing `instant-acme` from `crates/portal-acme/Cargo.toml` (commit `5898103`) reflects the same gap: the dep re-lands when the wrapper does.
...
- **Phase 6a — partial.** Per [`docs/plans/2026-05-04-006-feat-portal-sdk-plan.md`](docs/plans/2026-05-04-006-feat-portal-sdk-plan.md). Landed: scaffold + `error::SdkError` + `events` (TunnelState/TunnelEvent + broadcast channels) + `relay_set` + `picker` (eclipse-resistant ASN-bin coverage) + `mitm` (RFC 5705 EKM probe). Pending: U6 `expose.rs` (ExposeSession lifecycle), U7 `listener.rs` (accept loop + channel dispatch), U8 `identity.rs` (tenant-key loader thin re-export). PLAN.md previously marked Phase 6a "landed" reflecting the task-list view; the actual `crates/portal-sdk/src/` module set is `error`, `events`, `mitm`, `picker`, `relay_set` — no `expose`/`listener`/`identity` modules — so Phase 6a is partial-landed against the plan's full surface.
...
- **Concurrency invariant**: every spawned task lives inside a `tokio::task::JoinSet` or carries a `tokio_util::sync::CancellationToken`. Free `tokio::spawn` permitted only at top-level `main` or at sites satisfying the OR clause via stored `JoinHandle` + `CancellationToken`. Enforced workspace-wide by clippy `disallowed_methods` on `tokio::spawn` (`clippy.toml`); audited sites carry statement-level `#[expect(clippy::disallowed_methods, reason = "...")]` naming the architectural justification (top-of-main, lifecycle-collapsing detached drain, R9 OR clause, or test code). See `docs/architecture.md` §Structured concurrency invariant.
- **Secret invariant**: every private key, admin token, lease secret stored in `secrecy::SecretBox<T>` with role-specific newtype `T`. Each role has exactly one `load_*_key` constructor; consumers accept only the role-typed newtype. No public API produces or consumes bare `String` for secret material — a contributor introducing one is caught at code review (no silent escape hatch from the typed surface). A workspace-wide clippy `disallowed_types` rule on `String` is intentionally NOT a gate (impractical — `String` is used legitimately throughout); the type-system + single-loader-per-role discipline is the load-bearing mechanism.
