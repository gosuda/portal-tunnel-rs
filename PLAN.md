...
- `quic-go/quic-go` → `**quinn@0.C). HTTP/3 via` h3-quinn`deferred to Phase 5 evaluation — h3 is still 0.0.x and self-described as "experimental"; HTTP/2+HTTP/1.1 is the guaranteed surface, H3 opt-in once the`h3-*` family stabilizes.
...
- Configuration loading (Go env+flag merge) → `**figment`** (typed, layered: env + JSON file + CLI f* facade + `**metrics-exporter-prometheus`** for `/metrics` endpoint
...
- **Time**: every `Instant`/`SystemTime` boundary mediated through `jiff:ss; UTC always explicit.
...
- **HTTP stack**: `axum@0.8.9` + `hyper 1.x` + `tower`. HTTP/1.1 + HTTP/2 baseline; HTTP/3 (via `h3-quinn`) deferred to Phase 5 evaluation since `h3@0.0.8` and `h3-quinn@0.0.10` remain pre-1.0 and "expetrust-boundary `rustls::ServerConfig` instances served from three distinct `axum::Router` mounts (R2).
...
- **F1** — Svelte regen pipeline: utoipa-generated TS client lands in Go repo's frontend dir, frontend rebuilds, dist artifacts re-committed tI gate on TS-client drift between Rust API and committed Svelte bundle.
...
**Goalss signing server (rustls `SigningKey` async-bridged via tokio channel + worker pool — FEAS-2), WireGuard hop-mux overlay. **R10 cross-relay reputation propagation, hop-mux per-hop accounting, and `ReputationDelta` wire envelope are deferred to v0.2** (see v0.2 Backlog).
...
| `boringtun` upstream restructuring leaves us picking a fork                                                                                                                                                             | Phase 6b plan evaluates Mullvad GotaTun, `defguard_boringtun`, `wiresock/boringtun` against current state at decision time; ADR records the rationale; fallback is to vendor wireguard-go semantics into a pure-Rust port.                                                                                                                                                                                                                                 |
...
- **Performance benchmarks vs Go v2.1.8** — promoted from "Deferred to Follow-Up Work" to v0.2 first-class deliverable per product-lens P2-R2-7. Without this, the "modern reference implementation" claim has no v0.1 evidence path. **Trigger:** v0.1 ships.
...
- **utoipa coverage CI gate** (full implementation) — v0.1 lands the clippy `disallowed_methods` ban on bare `axum::Router::route` + ast-grep CI scan per FEAS-R2-7; v0.2 adds the `xtask openapi-export` snapshot test against committed `docs/openapi.yaml`. **Trigger:** v0.1 ships.
