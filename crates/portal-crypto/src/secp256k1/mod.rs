...
//!
//! - **[`key`]** — the [`key::TenantSecp256k1Key`] newtype and its sole constructor
//!   [`key::load_tenant_secp256k1_key`].  Key material is held behind
//!   `secrecy::SecretBox<TenantSecp256k1Key>` so secrets are zeroized on drop.
//!
//! - **[`address`]** — [`address::EthAddress`] newtype with EIP-55 mixed-case checksum
//!   `Display` impl, and [`address::evm_address_from_pubkey`] which derives the EVM
//!   address from a secp256k1 public key via Keccak-256.
//!
//! - **[`eip191`]** — [`eip191::sign_eip191_personal`] which produces a 65-byte
