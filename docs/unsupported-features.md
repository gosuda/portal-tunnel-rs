...
| Production-grade relay mesh/multi-hop parity | Experimental only | Overlay identity, discovery metadata, peer config, `tokio-wireguard`, and HopMux are wired, but multi-hop mesh mode still lacks relay-pair smoke coverage (blocked: discovery HTTP client uses default reqwest TLS, rejecting self-signed/runtime peer certificates), NAT/keepalive validation, MTU validation, peer churn testing, and Go v2.1.8 mesh interop validation. Treat it as not production-supported. |
